...
object Recipes {
  ...
  def writeToTopic(topic: String, values: Iterable[Supported]): ZIO[Scope with DynamicProducer, Throwable, Unit] =
    for {
  ...
  def pipeToTopic(values: ZStream[Any, Throwable, Supported], topic: String | Null = null): ZIO[Scope with DynamicProducer, Throwable, Unit] =
    for {
      writer <- ZIO.service[DynamicProducer]
      _ <- values.run(
  ...
  /**
    ...
    * @return
    */
  def pipeToTopicWithKeys[K <: Supported, V <: Supported](values: ZStream[Any, Throwable, V])(asKey: V => K): ZIO[Scope with DynamicProducer, Throwable, Unit] =
    for {
      writer <- ZIO.service[DynamicProducer]
      _ <- values.run(
  ...
  /**
    ...
    */
  def copyTopic[K <: Supported, V <: Supported](fromTopic: String)(mapRecord: KafkaRecord => ProducerRecord[K, V] = _.asProducerRecord())
    : ZIO[Scope with DynamicProducer with BatchedStream, Nothing, Fiber.Runtime[Throwable, Unit]] = {
    for {
