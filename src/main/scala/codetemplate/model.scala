...
/**
  ...
  * @param record some message input
  * @param env a system environment
  * @param fs a handle to a file system like thing
  * @tparam A the record (message) type
  */
...
case class FileSystem(dir: Path) extends AnyVal with Dynamic {
...
/**
  ...
  * @tparam V
  */
case class Message[K, V](content: V,
                         key: K,
                         timestamp: Long = 0,
